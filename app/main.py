...
def seed(db: Session):
    ...
    if not a:
        db.add(User(email=DEFAULT_ADMIN_EMAIL, password_hash=hash_password(DEFAULT_ADMIN_PASSWORD), is_admin=True))
        db.commit()

    # ---- LENDER DIRECTORY (DIRECT LENDERS) ----
    # Notes:
    # - "direct" here means the destination where you apply and get underwritten by that lender/provider.
    # - You can edit/disable any lender in Admin → Lender Directory.
    seed_lenders = [
        # Existing + core
        ...
        # Additional GA-focused / Southeast / National direct lenders (directory expansion)
        ...
        # More direct lenders (bulk expansion to hit +50 real estate lenders total)
    ...
    # Business direct lenders/providers (30 total target)
    seed_business = [
    ...
    # Personal direct lenders/providers (20 total target)
    seed_personal = [
    ...
    seed_lenders.extend(seed_business)
    seed_lenders.extend(seed_personal)
    ...
    # ---- PRODUCT BOXES (ONE PER LENDER, EDITABLE IN UI) ----
    # This makes "matched with" explicit: the match list is the set of eligible products (and their lenders).
    def slug(s: str) -> str:
        import re
        s=s.upper()
    ...
    def add_product(key, name, category, priority, lender_name, notes, cfg):
        if db.query(Product).filter(Product.key==key).first():
            return
        db.add(Product(key=key, name=name, category=category, priority=priority, lender_name=lender_name, notes=notes,
                       config_json=json.dumps(cfg, indent=2), sources_json=json.dumps([{"type":"matrix","url": ""}], indent=2), last_verified_at=datetime.utcnow(), verify_interval_days=30, is_active=True))

    # Real estate: asset-based first, then light-PG 600+
    # We'll generate products for each real_estate lender entry.
    re_lenders=db.query(Lender).filter(Lender.categories.like("%real_estate%")).all()
    pr=10
    for l in re_lenders:
        k=f"RE_{slug(l.name)}"
        # Heuristic: DSCR-oriented if name contains DSCR/RENTAL/COREVEST/VISIO/NEWFI/LENDINGONE
        nm=l.name.lower()
        if any(t in nm for t in ["dscr","rental","corevest","visio","newfi","lendingone"]):
            cfg={"requires":["credit_score","arv","market_rent","taxes","insurance"],
            ...
            cfg={"requires":["purchase_price","rehab_budget","arv"],
    ...
    # Business: generate products for each business lender
    bus_lenders=db.query(Lender).filter(Lender.categories.like("%business%")).all()
    pr=20
    for l in bus_lenders:
        k=f"BUS_{slug(l.name)}"
        cfg={"requires":["fico","time_in_business_months","avg_monthly_revenue"],
             "rules":{"min_fico":600,"min_time_in_business_months":6,"min_avg_monthly_revenue":10000,
                      "approve_score":60,"conditional_score":40,"tib_norm":36,"revenue_norm":300000},
...
def client_register_post(request: Request, full_name: str=Form(...), email: str=Form(...), phone: str=Form(None), password: str=Form(...), db: Session=Depends(get_db)):
...
def client_dashboard(request: Request, db: Session=Depends(get_db)):
    u=current_user(request, db)
    if not u: return RedirectResponse("/client/login", status_code=303)
    ...
    for d in decs:
        ...
        rows.append({"created_at": d.created_at.strftime("%Y-%m-%d %H:%M"),
                     "category": sub.category if sub else "",
                     "top_product": d.top_product_key, "status": d.top_status, "score": d.top_score,
...
def intake_post(
    ...
    payload={
        "fico": fico, "credit_score": fico,
        "monthly_income": monthly_income,
        "annual_income": annual_income,
        "monthly_debt": monthly_debt,
        "entity_type": entity_type, "industry": industry,
        "time_in_business_months": time_in_business_months,
        "annual_revenue": annual_revenue,
        "avg_monthly_revenue": avg_monthly_revenue,
        "address": address, "property_type": property_type,
        "purchase_price": purchase_price, "rehab_budget": rehab_budget, "arv": arv,
        "market_rent": market_rent, "section8_rent": section8_rent,
        "taxes": taxes, "insurance": insurance, "hoa": hoa,
        "mgmt_pct": 0.13, "vacancy_pct": 0.08, "capex_pct": 0.05
    }

    sub=Submission(user_id=u.id, category=category, client_name=client_name, client_email=client_email, client_phone=client_phone,
    ...
    for l in db.query(Lender).filter(Lender.is_active==True).all():
        ...
        if category in cats:
            lenders.append({"name": l.name, "website": l.website, "geography": l.geography, "contact_notes": l.contact_notes})

    if top and top.get("lender_name") and not any(x["name"]==top["lender_name"] for x in lenders):
        lenders.insert(0, {"name": top.get("lender_name"), "website":"", "geography":"", "contact_notes":"Add lender details in Admin → Lender Directory."})
    ...
    build_pdf(pdf_path, {"category":category,"client_name":client_name,"client_email":client_email,"client_phone":client_phone}, top, ranking, lenders)

    dec=Decision(submission_id=sub.id, user_id=u.id,
    ...
    # Matched-with = lenders attached to eligible products (APPROVED/CONDITIONAL) in ranked order
    matched_with=[]
    for r in ranking:
...
def admin_home(request: Request, db: Session=Depends(get_db)):
    if not current_admin(request, db): return RedirectResponse("/admin/login", status_code=303)
...
def lenders_new_post(request: Request, name: str=Form(...), website: str=Form(None), geography: str=Form(None), categories: str=Form(None), contact_notes: str=Form(None), db: Session=Depends(get_db)):
    if not current_admin(request, db): return RedirectResponse("/admin/login", status_code=303)
    if db.query(Lender).filter(Lender.name==name).first(): raise HTTPException(status_code=400, detail="Lender name exists")
...
def lenders_edit_post(request: Request, lid: int, website: str=Form(None), geography: str=Form(None), categories: str=Form(None), contact_notes: str=Form(None), db: Session=Depends(get_db)):
    if not current_admin(request, db): return RedirectResponse("/admin/login", status_code=303)
    l=db.query(Lender).filter(Lender.id==lid).first()
    if not l: raise HTTPException(status_code=404, detail="Not found")
    l.website=website; l.geography=geography; l.categories=categories; l.contact_notes=contact_notes
    db.commit()
    return RedirectResponse("/admin/lenders", status_code=303)
...
def products_list(request: Request, db: Session=Depends(get_db)):
    ...
    # stale if last_verified_at older than verify_interval_days
    now=datetime.utcnow()
    for p in products:
...
def products_new(request: Request, db: Session=Depends(get_db)):
    ...
    return templates.TemplateResponse("admin/products_edit.html", {"request": request, "p": None, "config_json": default_cfg, "sources_json": "[]", "last_verified_display": "—", "is_new": True, "action": "/admin/products/new"})

@app.post("/admin/products/new")
def products_new_post(request: Request, key: str=Form(...), name: str=Form(...), category: str=Form(...), priority: int=Form(50), lender_name: str=Form(None), notes: str=Form(None), config_json: str=Form(...), sources_json: str=Form(None), verify_interval_days: int=Form(30), db: Session=Depends(get_db)):
    if not current_admin(request, db): return RedirectResponse("/admin/login", status_code=303)
    if db.query(Product).filter(Product.key==key).first(): raise HTTPException(status_code=400, detail="Key exists")
    cfg=json.loads(config_json)
    db.add(Product(key=key, name=name, category=category, priority=priority, lender_name=lender_name, notes=notes,
                   config_json=json.dumps(cfg, indent=2), sources_json=sources_json, verify_interval_days=verify_interval_days,
                   last_verified_at=datetime.utcnow(), is_active=True))
    db.commit()
...
def products_edit(request: Request, pid: int, db: Session=Depends(get_db)):
    ...
    return templates.TemplateResponse("admin/products_edit.html", {"request": request, "p": p, "config_json": p.config_json, "is_new": False, "action": f"/admin/products/edit/{pid}"})

@app.post("/admin/products/edit/{pid}")
def products_edit_post(request: Request, pid: int, name: str=Form(...), category: str=Form(...), priority: int=Form(50), lender_name: str=Form(None), notes: str=Form(None), config_json: str=Form(...), sources_json: str=Form(None), verify_interval_days: int=Form(30), db: Session=Depends(get_db)):
    if not current_admin(request, db): return RedirectResponse("/admin/login", status_code=303)
    p=db.query(Product).filter(Product.id==pid).first()
    if not p: raise HTTPException(status_code=404, detail="Not found")
    cfg=json.loads(config_json)
    p.name=name; p.category=category; p.priority=priority; p.lender_name=lender_name; p.notes=notes; p.config_json=json.dumps(cfg, indent=2); p.sources_json=sources_json; p.verify_interval_days=verify_interval_days
    db.commit()
    return RedirectResponse("/admin/products", status_code=303)
